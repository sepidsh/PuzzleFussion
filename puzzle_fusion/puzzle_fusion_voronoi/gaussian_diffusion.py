...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        model_output_dec, _ = model(x, self._scale_timesteps(t), xtalpha=xtalpha, epsalpha=epsalpha, is_syn=True, **model_kwargs)
    ...
    def training_losses(self, model, x_start, t, model_kwargs, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_mean_type == ModelMeanType.EPSILON:
                pred_xstart = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output_dec).float()
            elif self.model_mean_type == ModelMeanType.START_X:
                pred_xstart = model_output_dec
            else:
                assert False
            rot_mat = th.cat([pred_xstart[:,2:,:].permute([0,2,1]).unsqueeze(2), th.flip(pred_xstart[:,2:,:],dims=(1,)).permute([0,2,1]).unsqueeze(2)], axis=2)
            rot_mat = rot_mat * th.tensor([[1, -1],[1,1]], device=rot_mat.device)
            new_polys = th.bmm(rot_mat.reshape([-1, 2, 2]), model_kwargs['poly'].reshape([-1, 2, 1]).float()).view_as(model_kwargs['poly']).permute([0,2,1])
            final_polys = pred_xstart[:,:2,:] + new_polys
            final_polys = final_polys.permute([0,2,1])
            poly_set1 = final_polys[th.arange(final_polys.shape[0])[:, None], model_kwargs['rels'][:,:,0].long()]
            poly_set2 = final_polys[th.arange(final_polys.shape[0])[:, None], model_kwargs['rels'][:,:,1].long()]
            tmp_mask_pairwise = (model_kwargs['rels'].sum(2)>0).long()
            terms["mse_pairwise"] = mean_flat(((poly_set1 - poly_set2) ** 2).permute([0,2,1]) * (t.unsqueeze(1).unsqueeze(1)<500), tmp_mask_pairwise)
            ...
            if "vb" in terms:
                ...
                terms["loss"] = terms["mse_dec"] + terms["mse_pairwise"]
        else:
